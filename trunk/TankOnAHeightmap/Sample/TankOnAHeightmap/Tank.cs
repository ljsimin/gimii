...
namespace TanksOnAHeightmap
{
    class Tank
    {
            ...
            KeyboardState currentKeyboardState, HeightMapInfo heightMapInfo)
        {
            ...
            if(currentKeyboardState.IsKeyDown(Keys.D1)|| (kontroler!=null && kontroler.Stanje.Dugmici.JEDAN))
            {
                mode = 0;
                System.Console.WriteLine("Mode 0");
            }
            if (currentKeyboardState.IsKeyDown(Keys.D2) || (kontroler != null && kontroler.Stanje.Dugmici.DVA))
            {
                mode = 1;
                System.Console.WriteLine("Mode 1");
            }
            ...
            if (kontroler != null)
            {
                ...
                if (mode == 0)
                {
                    if (kontroler.Stanje.Akcelerometar.X > 0.5f)
                    {
                        turnAmount -= 1;
                    }
                ...
                {
                    if (Math.Abs(kontroler.Stanje.Akcelerometar.Y) > 0.1f)
                    {
                        //if ((kontroler.Stanje.Akcelerometar.Z > 0 && kontroler.Stanje.Akcelerometar.X < 0) || (kontroler.Stanje.Akcelerometar.Z < 0 && kontroler.Stanje.Akcelerometar.X > 0))
                        if ((kontroler.Stanje.Akcelerometar.Z > 0))
                            turnAmount += kontroler.Stanje.Akcelerometar.Y;
                        else
                            turnAmount -= kontroler.Stanje.Akcelerometar.Y;
                    }
                }
            ...
            if (kontroler != null)
            {
                if (mode == 0)
                {
                    if (kontroler.Stanje.Akcelerometar.Y > 0.3f)
                    {
                        movement.Z = -1;
                        accelerating = true;
                    }
                ...
                {
                    if (Math.Abs(kontroler.Stanje.Akcelerometar.Z) > 0.1f)
                    {
                        movement.Z = -(kontroler.Stanje.Akcelerometar.Z ) * 2;
                    }
                }
            }
        ...
        public double getDistance()
        {
            ...
            if (kontroler != null)
            {
                if (kontroler.Stanje.Dugmici.A == true)
                {
                    if (kontroler.Stanje.Senzori[0].Nadjen && kontroler.Stanje.Senzori[1].Nadjen)
                    {
                        if (distance < 0)
                        {
                            distance = Math.Sqrt(Math.Pow((kontroler.Stanje.Senzori[1].x - kontroler.Stanje.Senzori[0].x), 2) + Math.Pow((kontroler.Stanje.Senzori[1].y - kontroler.Stanje.Senzori[0].y), 2));
                        }
                        else
                        {
                            double newDistance = Math.Sqrt(Math.Pow((kontroler.Stanje.Senzori[1].x - kontroler.Stanje.Senzori[0].x), 2) + Math.Pow((kontroler.Stanje.Senzori[1].y - kontroler.Stanje.Senzori[0].y), 2));

                            if (Math.Abs(distance - newDistance) > 0.005f)
                            {
                                result = cameraDistance + (distance - newDistance)*3000;
